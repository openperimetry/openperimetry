 ...
 * the screen limits.
 *
 * We now compute the rectangle area — (screenWidth × screenHeight) / ppd² —
 * using the calibration recorded at test time. When the caller doesn't
 * supply full calibration (legacy call sites, pre-0.3.0 OVFX imports with
 * no stored screen dimensions), we fall back to 4 × maxEccentricityDeg²
 * which approximates a square screen where maxEcc is half the side.
 */
function expectedNormalArea(
  ...
  // Fallback: treat maxEcc as half the diagonal of a square screen.
  return 4 * maxEccentricityDeg * maxEccentricityDeg
}
...
export function Interpretation({ points, areas, maxEccentricityDeg, calibration }: Props) {
